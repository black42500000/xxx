...
namespace Conquer_Online_Server.Game.Features
{
    public class SpiritBeadQuest
    {
        ...
        public SpiritBeadQuest(GameClient c)
        {
            Client = c;
        }
        ...
        {
            if(Client != null)
            new Database.MySqlCommand(Conquer_Online_Server.Database.MySqlCommandType.UPDATE).Update("entities").Set(column, value).Where("UID", Client.Entity.UID).Execute();
        }
            ...
            if (CanAccept)
            {
                if (Bead == 0)
                {
                    ...
                    if (Database.ConquerItemInformation.BaseInformations.TryGetValue(_Bead, out CIBI))
                    {
                        ...
                        i.ID = CIBI.ID;
                        i.UID = ConquerItem.ItemUID.Next;
                        i.Durability = CIBI.Durability;
                        i.MaximDurability = CIBI.Durability;
                        i.Color = (Conquer_Online_Server.Game.Enums.Color)Kernel.Random.Next(4, 8);
        ...
        private UInt32 Requiredspirits
        {
            get
            {
                switch (Bead)

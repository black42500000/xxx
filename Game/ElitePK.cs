...
namespace Conquer_Online_Server.Game
{
    public class ElitePK
    {
        ...
        public class Match
        {
                    ...
                    if (TotalWagers != 0)
                    {
                        ...
                        foreach (var kvp in target.ElitePKStats.Wagers)
                        {
                            ...
                            if (Kernel.GamePool.TryGetValue(kvp.Key, out pClient))
                            {
                                ...
                                Conquer_Online_Server.Kernel.SendWorldMessage(new Message("Congratulations! " + pClient.Entity.Name + " has defeated " + target.ElitePKStats.Name + " in ElitePK.", System.Drawing.Color.Black, Message.TopLeft), Program.GamePool);
                            }
                        }
            ...
            private GameClient targetOf(GameClient client)
            {
                for (int i = 0; i < Players.Length; i++)
                    if (Players[i] != null)

...
namespace Conquer_Online_Server.Game.Features.Kisses
{
    ...
    {// kisses system fixed but he dont insert into table idk why :D jason
        private KissPacket Packet;
        public KissSystem(byte[] BasePacket, Client.GameClient Caller)
        {
            ...
            if (Packet.ItemUID == 0)
            {
                ...
                if (Caller.Entity.Kisses.LastKissesSent.AddDays(1) <= DateTime.Now)
                {
                    ...
                    Database.KissesystemTable.SaveKissTable(Kernel.GamePool[Packet.UID1]);
                    Database.KissesystemTable.SaveKissTable(Caller);
                }
                else
                ...
                if (Caller.Inventory.TryGetValue(Packet.ItemUID, out Item))
                {
                    ...
                    if (Kisses != KissType.Unknown)
                    {
                        switch (Kisses)
                        {
                            ...
                            case KissType.Letters: Kernel.GamePool[Packet.UID1].Entity.Kisses.Letters += Item.Durability; Kernel.GamePool[Packet.UID1].Entity.Kisses.Letters2day += Item.Durability; break;

...
namespace Conquer_Online_Server.Game
{
    public class Champion
    {
            ...
            public class QualifierGroup
            {
                ...
                public void Import()
                {
                    ...
                    if (Player1.Map.ID == Player2.Map.ID)
                    {
                        ...
                        Player2.Send(ChampionKernel.OpponentInfo(Player1.ChampionStats).BuildPacket());
                        Player1.Send(ChampionKernel.Stats(0, 0).BuildPacket());
                        Player2.Send(ChampionKernel.Stats(0, 0).BuildPacket());
                        Player1.Send(Player1.ChampionStats);
                        Player2.Send(Player2.ChampionStats);
            ...
            public static void DoGiveUp(Client.GameClient client)
            {
                ...
                else
                {
                    if (client.ChampionStats.Opponent != null)
                    {
                        ...
                        {
                            WaitingPlayerList.Remove(client.Account.EntityID);
                            WaitingPlayerList.Remove(client.ChampionStats.Opponent.Account.EntityID);
                        }
                        if (client.QualifierGroup != null)
                        {
        ...
        {
            public static void ShowTop(Client.GameClient client, int type = 2602)
            {
            ...
            public static void ShowRankingPage(ushort group, int pageIndex, Client.GameClient client)
            {
                MemoryStream strm = new MemoryStream();
                BinaryWriter wtr = new BinaryWriter(strm);
                wtr.Write((ushort)0);
        ...
        public static void VerifyAwaitingPeople()
        {
            foreach (var Challanger in WaitingPlayerList.Values)
            {
                if (Challanger != null)
                {
                    ...
                    {
                        Client.GameClient Challanged = Challanger.ChampionStats.Opponent;
